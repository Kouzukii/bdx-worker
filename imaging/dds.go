...
func TextureFromDDS(fname string) (uint32, error) {
	file, err := os.Open(fname)
	if err != nil {
		return 0, fmt.Errorf("Cannot open DDS file: %v", err)
	}
	defer file.Close()
	...
	if string(filecode[:]) != "DDS " {
		return 0, fmt.Errorf("File code is not DDS, instead got %v.", string(filecode[:]))
	}

	header := ddsHeader{}
	err = binary.Read(file, binary.LittleEndian, &header)
	if err != nil {
		return 0, fmt.Errorf("Couldn't read DDS header: %v", err)
	}

	stat, err := file.Stat()
	if err != nil {
		return 0, fmt.Errorf("Couldn't get size of file: %v", err)
	}
	...
	if err != nil {
		return 0, fmt.Errorf("Couldn't read all mipmaps into buffer: %v", err)
	}
	...
	case fourCC_DXT5:
		...
		blockSize = 16
	default:
		return 0, fmt.Errorf("Invalid four CC in DDS header. Got: %x; Expected %x; %x; or %x", header.PixelFormat.FourCC, fourCC_DXT1, fourCC_DXT3, fourCC_DXT5)
	}
	...
	for level, width, height := 0, header.Width, header.Height; level < int(header.MipMapCount) && (width > 0 || height > 0); level, width, height = level+1, width/2, height/2 {
		...
		gl.CompressedTexImage2D(gl.TEXTURE_2D, int32(level), format, int32(width), int32(height), 0, size, &buffer[offset])
